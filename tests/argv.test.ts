import { describe, expect, test } from "vitest";
import { prepareServeArgv, resolveCliName, resolveServeArgv } from "../src/lib/argv.js";

describe("resolveCliName", () => {
  test("uses x-mcp for x-mcp invocation help", () => {
    expect(resolveCliName("/usr/local/bin/x-mcp")).toBe("x-mcp");
  });

  test("defaults to x-mcp for direct source execution", () => {
    expect(resolveCliName("/repo/src/index.ts")).toBe("x-mcp");
  });
});

describe("resolveServeArgv", () => {
  test("defaults bare x-mcp invocation to MCP stdio mode", () => {
    expect(resolveServeArgv("/usr/local/bin/x-mcp", [])).toEqual(["--mcp"]);
  });

  test("uses shell command path when defaulting bare x-mcp to MCP stdio mode", () => {
    expect(resolveServeArgv("/repo/dist/index.js", [], "/usr/local/bin/x-mcp")).toEqual(["--mcp"]);
  });

  test("does not override explicit x-mcp arguments", () => {
    expect(resolveServeArgv("/usr/local/bin/x-mcp", ["--help"])).toEqual([
      "--help",
    ]);
  });

  test("does not treat legacy x-cli as a recognized bin", () => {
    expect(resolveServeArgv("/usr/local/bin/x-cli", [])).toEqual([]);
  });
});

describe("prepareServeArgv", () => {
  test("strips auth json and preserves mcp serve argv", () => {
    expect(
      prepareServeArgv("/usr/local/bin/x-mcp", [
        "--mcp",
        "--auth-json",
        "{\"type\":\"x-mcp-auth\"}",
        "--mcp-mode",
        "read-write",
      ]),
    ).toEqual({
      argv: ["--mcp"],
      authJson: "{\"type\":\"x-mcp-auth\"}",
      mcpMode: "read-write",
    });
  });

  test("bare x-mcp with auth json still defaults to mcp mode", () => {
    expect(
      prepareServeArgv("/repo/src/index.ts", [
        "--auth-json",
        "{}",
      ], "/usr/local/bin/x-mcp"),
    ).toEqual({
      argv: ["--mcp"],
      authJson: "{}",
    });
  });

  test("extracts mcp export format", () => {
    expect(
      prepareServeArgv("/usr/local/bin/x-mcp", [
        "--mcp",
        "--export",
        "codex",
      ]),
    ).toEqual({
      argv: ["--mcp"],
      exportFormat: "codex",
    });
  });

  test("normalizes auth export --format to positional format", () => {
    expect(
      prepareServeArgv("/usr/local/bin/x-mcp", [
        "auth",
        "export",
        "--format",
        "claude",
      ]),
    ).toEqual({
      argv: ["auth", "export", "claude"],
    });
  });
});
