...
You are an AI agent working with the x-mcp codebase. This file summarizes the project shape and the important safety constraints.

## What This Is

x-mcp is a TypeScript CLI and MCP stdio server built with [incur](https://github.com/wevm/incur) for X/Twitter API v2. Runtime target is Node.js 20+. The single public bin is `x-mcp`.

A bare `x-mcp` invocation is treated as `x-mcp --mcp`. Explicit arguments such as `x-mcp --help` or `x-mcp tweet search "AI"` run normal CLI commands.
...
src/
  ...
  lib/
    ...
    tokens.ts           -- Token storage (~/.x-mcp/tokens.json)
...
- `x-mcp` with no args starts MCP stdio mode.
...
- Runtime MCP auth can be provided through `X_MCP_AUTH_JSON` or `--auth-json`; environment transport is preferred.
- Local config lives under `~/.x-mcp/config.json`; tokens live under `~/.x-mcp/tokens.json`.
...
| Error | Cause | Fix |
|-------|-------|-----|
| "Not logged in" | No tokens found | Run `x-mcp auth login` |
| Token refresh failed | Refresh token expired | Run `x-mcp auth login` again |
| 401 Unauthorized | Bad or expired tokens | Check `x-mcp auth status`, re-login if needed |
