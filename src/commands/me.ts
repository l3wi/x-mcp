...
me.command("mentions", {
  ...
  async run(c) {
    ...
    return paginateCursor(c.options, (request) => client.getMentions(request));
  },
});
...
me.command("bookmarks", {
  ...
  options: z.object({
    limit: z.number().default(10).describe("Total results to return"),
    pageSize: z.number().default(10).describe("Results per API request (1-100)"),
    cursor: z.string().optional().describe("X pagination cursor to start from"),
    offset: z.number().default(0).describe("Results to skip by walking cursor pages"),
  }),
  async run(c) {
    const client = createClient(c.env);
    return paginateCursor(c.options, (request) => client.getBookmarks(request));
  },
});
