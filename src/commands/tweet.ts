...
export function createTweetCommand(options: TweetCommandOptions = {}) {
  const { includeWrite = true } = options;
  const tweet = Cli.create("tweet", {
  ...
  tweet.command("context", {
    ...
    options: z.object({
      max: z
        .number()
        .default(10)
        .describe("Max recent conversation tweets (10-100)"),
    }),
    async run(c) {
      const client = createClient(c.env);
      const tid = parseTweetId(c.args.id);
  ...
  tweet.command("search", {
    ...
    options: z.object({
      ...
      pageSize: z
        .number()
        .default(10)
        .describe("Results per API request (10-100)"),
...
export const tweet = createTweetCommand();
